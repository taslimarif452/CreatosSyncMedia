    ...
    >
      ...
      <div className="md:sticky md:top-0 md:h-screen w-full flex flex-col justify-center py-8 sm:py-12 overflow-hidden z-10">
        ...
        <div className="relative z-20 w-full max-w-7xl mx-auto px-6 text-center flex flex-col items-center select-none mb-6 sm:mb-8">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#121212] border border-[#262626] text-[10px] sm:text-[11px] font-mono font-bold text-[#4F7CFF] uppercase tracking-[0.25em] mb-3 shadow-sm">
            <span className="w-1.5 h-1.5 rounded-full bg-[#4F7CFF] animate-pulse" />
            <span>SERVICES</span>
          </div>

          <h2
            className="text-3xl sm:text-4xl md:text-5xl lg:text-[48px] font-extrabold tracking-tight text-[#F5F5F5] uppercase leading-[1.08]"
        ...
        <div className="relative z-20 w-full flex items-center my-auto overflow-hidden">
          {isMobile ? (
            /* Mobile: Native Touch Swipe Track showing ~1.1-1.3 cards with smooth snap */
            <div className="w-full flex items-stretch gap-4 px-6 overflow-x-auto no-scrollbar snap-x snap-mandatory py-3">
              {SERVICES_DECK.map((service, index) => (
                <div
                  key={service.id}
                  className="w-[285px] sm:w-[315px] flex-shrink-0 snap-start"
                >
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto pt-2">
        <h3
          className="text-xl sm:text-2xl font-extrabold text-[#F5F5F5] group-hover:text-white uppercase tracking-tight leading-tight mb-2.5 transition-colors duration-200"
          style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
        >
          {service.title}
        </h3>
        <p className="text-xs sm:text-sm text-[#959595] leading-relaxed font-normal">
          {service.description}
        </p>
      </div>
    </div>
